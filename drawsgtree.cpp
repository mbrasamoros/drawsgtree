...
long int ng[50] = {1, 1, 2, 4, 7, 12, 23, 39, 67, 118, 204, 343, 592, 1001, 1693, 2857, 4806, 8045, 13467, 22464, 37396, 62194, 103246, 170963, 282828, 467224, 770832, 1270267, 2091030, 3437839, 5646773, 9266788, 15195070, 24896206, 40761087, 66687201, 109032500, 178158289};

int pattern[MAX] = {1, 1, -1};
int patternlength = 3;
char patpolynomial[300] = "x_{1}+x_{2}-x_{3}";

bool limitnumberofnodes_set = false;
int numberofnodes = 0;
...
long int descendants(int N[], int G[], int S[], int c, int indexc, int g, int ming, int maxg, bool m_set, FILE *fout) {
  int i, s;
  int newN[MAX], newG[MAX], newS[MAX];
  int newc, newindexc;
  ...
  if (g <= maxg) {
    ...
    if (limitnumberofnodes_set && numberofnodes >= limitnumberofnodes)
      return 0;
    numberofnodes++;
...
long int ord_descendants(int N[], int G[], int M[], int F[], int c, int g, int m, FILE *fout) {
  int i, j, k, check;
  long int numdescendants = 0;
    ...
    if (M[i]) {
      ...
      for (j = 1; j < m; j++) {
        if (auxF[j]) {
          ...
          newF[i - j] = 0;
          if (j % 2 == 0 && (3 * j / 2 >= (i + 1) || !G[3 * j / 2 - 1]) && G[i - j / 2 - 1]) {
            check = 1;
    ...
    for (i = sc; i < c - 1; i++) {
      if (M[i]) {
        ...
        for (k = 0; k < m; k++)
          auxF[k] = F[k];
        checkf = c - 1 - i;
        if (!G[2 * checkf - 1] && (3 * checkf >= c || !G[3 * checkf - 1]) && (sc - checkf - 2 < 0 || G[sc - checkf - 2])) {
          check = 1;
...
void help() {
  ...
  std::cout << "  -visitlexgaps         visit nodes by lexicographic order of the gaps (defauls is -visitelxnongaps)" << std::endl;
  ...
  std::cout << "  -x <float>            scale the image by the specified factor" << std::endl;
  ...
  std::cout << "  -l <int>              limit the maximum number of nodes by the specified integer" << std::endl;
  ...
  std::cout << "           ./drawsgtree -g7 -n list -visitlexgaps -incremental" << std::endl;
  ...
  std::cout << "           ./drawsgtree -g7 -n gapseedbitstream -n list -plain -visitlexgaps" << std::endl;
  ...
  std::cout << "           ./drawsgtree -g10 -e infinitechains -visitlexgaps" << std::endl;
  std::cout << "           ./drawsgtree -g10 -e infinitechains -visitlexgaps -d 3." << std::endl;
  std::cout << "           ./drawsgtree -g42 -m6 -e infinitechains -visitlexgaps -etrim -d .2" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  int N[MAX], G[MAX], S[MAX], M[MAX], F[MAX];
  long int count[50];
  ...
  while ((j = getopt(argc, argv, ":hg:m:i:v:p:d:s:f:b:r:n:e:x:o:t:l:")) != -1) {
    switch (j) {
    case 'h':
      help();
      return 0;
    ...
    case 'm':
      ...
      break;
    case 'l':
      limitnumberofnodes = atoi(optarg);
      if (limitnumberofnodes < 1) {
        std::cerr << "Option -l must be followed by an integer in {1,2,...}"
                  << std::endl;
        return 1;
      }
      limitnumberofnodes_set = true;
      break;
    case 'i':
    ...
    case 'v':
      if (strcmp(optarg, "ertical") == 0)
        ...
        if (strcmp(optarg, "isitlexgaps") == 0)
          lexnongaps = false;
        else {
          if (strcmp(optarg, "isitlexnongaps") == 0)
            lexnongaps = true;
          else {
  ...
  if (m_set && ((m == 1 && maxg != 0) || (m > maxg + 1))) {
    std::cerr << "incompatible multiplicity and genus" << std::endl;
    return 1;
  }
  ...
  if (indexc) {
    ...
    if (m_set and m != N[1]) {
      std::cerr
          << "The given root semigroup does not have the specified multiplicity" << std::endl;
      return 1;
    } else {
  ...
  if (plain) {
    if (blackandwhite) {
      ...
      fprintf(fout,
              "\\providecommand\\dotscircles{}\\renewcommand\\dotscircles{\\scalebox{2.7}{{\\color{blue}\\dots}}}\n");
      if (optiongapseedbitstream) {
  ...
  for (j = initialj; j <= maxg; j++) {
    ...
    if (optionaperykunzposet)
      ...
      if (ordinarizationtree || quasiordinarizationforest) {
        fprintf(fout, "\\tikzset{level 1+/.style={level distance=%fcm}}", 2. * maxg);
      } else
    ...
    if (rotated) {
      if (scaled)
        fprintf(fout, "\\adjustbox{max width=%f\\textwidth,max height=%f\\textheight,angle=90}", .9 * scale, scale);
      else
        fprintf(fout, "\\adjustbox{max width=.9\\textwidth,max height=\\textheight,angle=90}");
    } else {
      if (scaled)
        fprintf(fout, "\\adjustbox{max width=%f\\textwidth,max height=%f\\textheight}", scale, .9 * scale);
      else
  ...
  if (limitnumberofnodes_set && numberofnodes >= limitnumberofnodes) {
    fout = fopen(filename, "w");
    fclose(fout);
    std::cerr << "NO GENERATED FILE. TOO MANY NODES." << std::endl;
    return 2;
  } else {
